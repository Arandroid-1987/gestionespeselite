...
public class VoceBilancioAdapterDay extends ArrayAdapter<VoceBilancio> {
	...
	public VoceBilancioAdapterDay(Activity context, int textViewResourceId,
			List<VoceBilancio> objects) {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (item instanceof RicavoProgrammato) {
			int color = context.getResources()
					.getColor(R.color.planned_incomes);
			layout.setBackgroundColor(color);
			holder.duplica.setBackgroundColor(color);
			holder.modifica.setBackgroundColor(color);
			holder.elimina.setBackgroundColor(color);
			holder.mostraTag.setTextColor(color);
		} else if (item instanceof SpesaProgrammata) {
			int color = context.getResources().getColor(R.color.planned_costs);
			layout.setBackgroundColor(color);
			holder.duplica.setBackgroundColor(color);
			holder.modifica.setBackgroundColor(color);
			holder.elimina.setBackgroundColor(color);
			...
			int color = context.getResources().getColor(R.color.costs);
			layout.setBackgroundColor(color);
			holder.duplica.setBackgroundColor(color);
			holder.modifica.setBackgroundColor(color);
			holder.elimina.setBackgroundColor(color);
			...
			int color = context.getResources().getColor(R.color.incomes);
			layout.setBackgroundColor(color);
			holder.duplica.setBackgroundColor(color);
			holder.modifica.setBackgroundColor(color);
			holder.elimina.setBackgroundColor(color);
	...
	private void duplicaVoce(VoceBilancio item, int position) {
		if (proVersion) {
	...
	private void modificaVoce(VoceBilancio item, int position) {
		if (proVersion) {
			context.setResult(Activity.RESULT_OK);
			context.finish();
			Intent intent;
	...
	private void eliminaVoce(VoceBilancio item, int position) {
		context.setResult(Activity.RESULT_OK);
		context.finish();
		...
		try {
			if (item instanceof Spesa) {
				if (item instanceof SpesaProgrammata) {
					if (SpesaProgrammataDao.deleteSpesa(db, item.getId())) {
					} else {
						toast = customToast.getErrorToast();
						toast.show();
					}
				} else {
					SpesaProgrammata s = SpesaProgrammataDao.getSpesa(db,
							item.getId());
					if (s != null) {
				...
				if (item instanceof RicavoProgrammato) {
					if (RicavoProgrammatoDao.deleteRicavo(db, item.getId())) {
					} else {
						toast = customToast.getErrorToast();
						toast.show();
					}
				} else {
					RicavoProgrammato r = RicavoProgrammatoDao.getRicavo(db,
							item.getId());
					if (r != null) {
